...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  createPlayheadObserversForMSE_(startTime) {
    ...
    regionObserver.addEventListener('enter', (event) => {
      ...
      this.sendEventStreamCallback_(region);
    });
  ...
  /**
   * If the region is an EventStream callback event
   * (schemeIdUri="urn:mpeg:dash:event:callback:2015"), fires a GET request to
   * the URL stored as the text content of the Event element.
   *
   * @param {shaka.extern.TimelineRegionInfo} region
   * @private
   */
  sendEventStreamCallback_(region) {
    if (region.schemeIdUri !== 'urn:mpeg:dash:event:callback:2015' ||
        !region.eventNode) {
      return;
    }
    const url = shaka.util.TXml.getTextContents(region.eventNode);
    if (!url) {
      return;
    }
    ...
    trimmedUrl = shaka.util.URL.applyUrlParams(trimmedUrl, region.urlParams);
    const type = shaka.net.NetworkingEngine.RequestType.EVENT_CALLBACK;
    const request = shaka.net.NetworkingEngine.makeRequest(
        [trimmedUrl],
        shaka.net.NetworkingEngine.defaultRetryParameters());
    this.networkingEngine_.request(type, request).promise.catch(() => {});
  }
